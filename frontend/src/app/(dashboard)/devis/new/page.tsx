...
import { Loader2, ArrowLeft, Plus, Trash2, ClipboardList, UserPlus, X } from "lucide-react";
    ...
    <div className="min-w-0 max-w-3xl space-y-6">
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">Créer un devis commercial</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit((d) => mutate(d))} className="space-y-5">
              ...
              <div key={field.id} className="grid grid-cols-1 sm:grid-cols-12 gap-2 items-start">
                <div className="sm:col-span-6">
                  ...
                  {errors.items?.[i]?.description && (
                    <p className="text-red-500 text-xs mt-1">{errors.items[i]?.description?.message}</p>
                  )}
                </div>
        ...
        <div className="card p-5 space-y-4">
          ...
          <div>
            <label className="label">Valide jusqu'au (optionnel)</label>
            <input
          ...
          <div>
            ...
            <textarea
              {...register("notes")}
              rows={3}
              placeholder="Conditions particulières, délai de réponse…"
